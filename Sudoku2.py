...
def print_missing_numbers_rowwise(original, solved):
    print("\nLiczby, które trzeba wpisać (po kolei, w każdym wierszu):")
    for i in range(9):
        row_chars = []
        for j in range(9):
            if original[i][j] == 0:
        ...
        print(f"Wiersz {i+1}: {row_str}")


# --- Główna część programu ---
filename = "C:\\Users\\konrd\\Desktop\\NAUKA\\Phyton\\sudoku.txt"  # wpisz pełną ścieżkę jeśli trzeba

board = read_board_from_file(filename)
original_board = [row[:] for row in board]  # kopia oryginalnej planszy
